...
- **Booking** — `userId`, `roomId`, `startTime`, `endTime`, `purpose`, `type` (studio_individuale · lezione · prova · concerto · altro), `status` (confirmed · cancelled · completed · no_show · pending_approval), `recurrenceId` (FK nullable a `BookingRecurrence`), `checkInToken` (UUID anti ghost-booking), `checkedInAt`, `autoCancelledAt`
- **BookingRule** — un record per ruolo: `maxActiveBookings`, `maxHoursPerWeek`, `maxHoursPerDay`, `min/maxBookingDurationMinutes`, `min/maxAdvance...`, `cancellationDeadlineHours`, `allowed/Start/EndTime`, `minIntervalBetweenBookingsMinutes` (cooldown), `allowRecurring`, ...
