...
| `20260514100000` | `oauth-allowed-domains`                            | `oauth_settings.allowedEmailDomains` (`STRING(1000)`, nullable): lista CSV di domini email autorizzati al login OAuth Google/Microsoft. NULL = nessuna restrizione                                                                                                                                                                   |
| `20260519100000` | `audit-log-hash-chain` (v1.11.0)                   | Hash-chain di integrità su `audit_log`: aggiunge `rowHash` + `prevHash` (`STRING(64)`, nullable per backward-compat con righe legacy). Il hook `beforeCreate` del modello popola le colonne sulle nuove righe; le pre-migration restano segnalate come `legacy` dalla verify ma non spezzano la catena. **Idempotente, no downtime** |
